...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", inline: <<-SHELL
    ...
    # Install Desktop and Virtualbox Guest Additions
    ...
    apt-get install -y bison build-essential cmake flex git libglib2.0-dev libgtk-3-dev libpcap-dev
...
EOF
      ...
      # Allow capturing without running wireshark as root
      setcap cap_net_raw,cap_net_admin+eip /home/vagrant/wireshark/build/run/dumpcap
      
      userdel ubuntu

      echo "VM Setup finished. Reboot it with 'vagrant reload' and login with:"
      echo "Username: vagrant"
      echo "Password: vagrant"
      echo
      echo "Then run Wireshark in a terminal with"
      echo "$ ./wireshark/build/run/wireshark-gtk"
  SHELL
end
